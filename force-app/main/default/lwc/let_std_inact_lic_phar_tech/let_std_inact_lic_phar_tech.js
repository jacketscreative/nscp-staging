...
export default class Let_std_inact_lic_phar_tech extends LightningElement {
	@api recordId;
	...
	async generatePdf(){
		const result = await getLetterOfStandingPharmTechController({currentRecordId: this.currentRecordId});
		...
		const tableDataBottom = [
			...
			["Initial Registration Date:", this.convertDate(result?.Initial_Registration_Date__c) ?? ''],
            // ["Current Status:", `${result?.Active__c == 'Yes' ? 'Active' : 'Inactive'}, ${result?.Type__c ?? ''},${result?.Class__c ? ` ${result?.Class__c},` : ''} Direct Patient Care.`],
            ["Current Status:", `${result?.Active__c == 'Yes' ? 'Active' : 'Inactive'}, ${result?.Type__c ?? ''}, ${result?.Class__c}`],
		...
		//Passage 1
		const para1 = `${result?.Registrant__r?.Name ?? ''} was a practicing registrant with the Nova Scotia College of Pharmacists and was entitled to practice pharmacy in a direct patient care setting without conditions in Nova Scotia until ${this.convertDate(result?.Expiry_Date__c) ?? ''}.`
		...
		doc.save("Letter of Standing (Inactive Registrant).pdf");
	}
